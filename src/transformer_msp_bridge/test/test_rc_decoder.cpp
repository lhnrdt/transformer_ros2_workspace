...
TEST(RcDecoder, RcChannelsBasic)
{
  ...
  auto sub = node->create_subscription<std_msgs::msg::UInt16MultiArray>("/msp/rc", 10,
                                                                        [&](std_msgs::msg::UInt16MultiArray::SharedPtr m)
                                                                        { last = m; });
  MSPPacket pkt;
  pkt.cmd = MSP_RC;
  // 8 channels: 1000..1700 step 100
  for (uint16_t v = 1000; v < 1800; v += 100)
  {
    pkt.payload.push_back(v & 0xFF);
    pkt.payload.push_back((v >> 8) & 0xFF);
  }
  decoder.decode(pkt);
  spin_for(node);
  ASSERT_TRUE(last);
...
TEST(RcDecoder, ShortPayloadIgnored)
{
  ...
  MSPPacket pkt;
  pkt.cmd = MSP_RC;
  pkt.payload = {0xE8, 0x03, 0xF4}; // 3 bytes (<16) should ignore
  decoder.decode(pkt);
  spin_for(node);
  EXPECT_FALSE(last);
}
