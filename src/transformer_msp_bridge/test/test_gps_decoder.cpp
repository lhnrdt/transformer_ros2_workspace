...
TEST(GpsDecoder, RawGpsBasic)
{
  ...
  auto sub_fix = node->create_subscription<sensor_msgs::msg::NavSatFix>(
      "/msp/gps/fix", 10, [&](sensor_msgs::msg::NavSatFix::SharedPtr m)
      { fix = m; });
  auto sub_vel = node->create_subscription<geometry_msgs::msg::TwistStamped>(
      "/msp/gps/vel", 10, [&](geometry_msgs::msg::TwistStamped::SharedPtr m)
      { vel = m; });
  MSPPacket pkt;
  pkt.cmd = MSP_RAW_GPS;
  ...
  { pkt.payload.push_back(v & 0xFF); pkt.payload.push_back((v>>8)&0xFF); };
  auto push32 = [&](int32_t v)
  { pkt.payload.push_back(v & 0xFF); pkt.payload.push_back((v>>8)&0xFF); pkt.payload.push_back((v>>16)&0xFF); pkt.payload.push_back((v>>24)&0xFF); };
  // fix flag, sats
  push8(1);
  push8(10);
...
TEST(GpsDecoder, CompGpsVector)
{
  ...
  auto sub_vec = node->create_subscription<geometry_msgs::msg::Vector3Stamped>(
      "/msp/home/vector", 10, [&](geometry_msgs::msg::Vector3Stamped::SharedPtr m)
      { home_vec = m; });
  auto sub_dist = node->create_subscription<std_msgs::msg::Float32>(
      "/msp/home/distance", 10, [&](std_msgs::msg::Float32::SharedPtr m)
      { home_dist = m; });
  MSPPacket pkt;
