...
static void spin_for(std::shared_ptr<rclcpp::Node> node, int ms = 60)
{
...
static DiagnosticStatus findStatus(const DiagnosticArray &arr, const std::string &name)
{
...
static std::string getValue(const DiagnosticStatus &st, const std::string &key)
{
...
TEST_F(SystemDecoderExtendedFixture, RcTuningDecoding)
{
  ...
  auto sub = node_->create_subscription<DiagnosticArray>("/msp/status_ex", 10, [&last](DiagnosticArray::ConstSharedPtr m)
                                                         { last = std::make_shared<DiagnosticArray>(*m); });
  MSPPacket pkt;
  pkt.cmd = MSP_RC_TUNING;
  pkt.payload = {100, 50, 55, 40, 10, 30, 90, 80, 70, 0x20, 0x03}; // tpa_breakpoint=0x0320=800
  decoder_->decode(pkt);
  spin_for(node_);
  ASSERT_TRUE(last);
...
TEST_F(SystemDecoderExtendedFixture, GpsStatisticsDecoding)
{
  ...
  auto sub = node_->create_subscription<DiagnosticArray>("/msp/status_ex", 10, [&last](DiagnosticArray::ConstSharedPtr m)
                                                         { last = std::make_shared<DiagnosticArray>(*m); });
  auto subSingle = node_->create_subscription<DiagnosticStatus>("/msp/gps/statistics", 10, [&single](DiagnosticStatus::ConstSharedPtr m)
                                                                { single = std::make_shared<DiagnosticStatus>(*m); });
  // errors=1, timeouts=2, pktCount=300 (0x012C), rejected=4, ignored=5, crc=6, gpsReset=0x89ABCDEF
  MSPPacket pkt;
  pkt.cmd = MSP_GPSSTATISTICS;
  pkt.payload = {
...
TEST_F(SystemDecoderExtendedFixture, RtcDecoding)
{
  ...
  MSPPacket pkt;
  pkt.cmd = MSP_RTC;
  pkt.payload = {0x04, 0x03, 0x02, 0x01, 0xD0, 0x07};
  decoder_->decode(pkt);
  spin_for(node_);
  ASSERT_TRUE(last);
