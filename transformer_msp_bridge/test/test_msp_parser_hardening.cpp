...
TEST(MSPParserHardening, MaxPayloadV1Rejected) {
  std::vector<MSPPacket> packets; MSPParser parser([&](const MSPPacket&p){ packets.push_back(p); });
  ...
  for (auto b: frame) parser.input(b);
...
TEST(MSPParserHardening, ZeroCopyPayloadDuringAccumulation) {
  ...
  for (size_t i=0;i<frame.size();++i) {
    ...
    if (i >= 6 && i < frame.size()-1) { // payload region before checksum
