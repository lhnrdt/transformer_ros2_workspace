...
TEST(SystemDecoder, StatusExBasic) {
  ...
  auto sub = node->create_subscription<diagnostic_msgs::msg::DiagnosticArray>("/msp/status_ex",10,
    [&](diagnostic_msgs::msg::DiagnosticArray::SharedPtr msg){ last = msg; });
  ...
  MSPPacket pkt; pkt.cmd = MSP_STATUS_EX; pkt.payload = {
    0xD0,0x07, // 2000
    0x03,0x00, // i2cErr
    0xFF,0x01, // sensors
  ...
  for (const auto &kv : st.values) {
    if (kv.key == "cycle_time_us" && kv.value == "2000") found_cycle = true;
    if (kv.key == "arming_flags" && kv.value == std::to_string(0x11223344)) found_arming = true;
  }
...
TEST(SystemDecoder, SensorStatusMasks) {
  ...
  MSPPacket pkt; pkt.cmd = MSP_SENSOR_STATUS; pkt.payload = { 0x0F,0x00, 0xF0,0x00 }; // present=0x000F failing=0x00F0
  decoder.decode(pkt);
  spin_for(node);
  ASSERT_TRUE(last);
  ASSERT_FALSE(last->status.empty());
  ...
  for (const auto &kv : st.values) {
    if (kv.key == "present_mask" && kv.value == std::to_string(0x000F)) present = true;
    if (kv.key == "failing_mask" && kv.value == std::to_string(0x00F0)) failing = true;
  }
  EXPECT_TRUE(present);
  EXPECT_TRUE(failing);
}
