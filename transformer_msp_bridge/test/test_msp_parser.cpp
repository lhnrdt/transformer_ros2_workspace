...
struct CapturedPacket { MSPPacket pkt; };

class MSPParserFixture : public ::testing::Test {
protected:
  void SetUp() override { packets_.clear(); parser_ = std::make_unique<MSPParser>([this](const MSPPacket &p){ packets_.push_back({p}); }); }
  std::unique_ptr<MSPParser> parser_; std::vector<CapturedPacket> packets_;
  void feed(const std::vector<uint8_t>& bytes){ for(auto b: bytes) parser_->input(b); }
};
...
TEST_F(MSPParserFixture, TunneledV2CrcFailure) {
  ...
  if(frame.size() > 7) { // ensure enough bytes
    ...
    for(size_t i = inner_start; i < inner_start + inner_len; ++i) cs ^= frame[i];
...
TEST_F(MSPParserFixture, PartialFrameNoEmit) {
  ...
  for(size_t i=0;i+1<frame.size();++i) parser_->input(frame[i]);
  EXPECT_EQ(packets_.size(), 0u);
}
...
TEST_F(MSPParserFixture, MultipleFramesSequence) {
  ...
  std::vector<uint8_t> all; all.insert(all.end(), f1.begin(), f1.end());
