...
TEST_F(InavStatusDecoderFixture, BasicDecoding) {
  ...
  auto sub2 = node_->create_subscription<transformer_msp_bridge::msg::MspInavStatus>(
    "/msp/inav_status", 10,
    [&received](transformer_msp_bridge::msg::MspInavStatus::ConstSharedPtr msg){ received = std::make_shared<transformer_msp_bridge::msg::MspInavStatus>(*msg); }
  );
  ...
  auto push16=[&](uint16_t v){ payload.push_back(v & 0xFF); payload.push_back((v>>8)&0xFF); };
  push16(cycle); push16(i2c); push16(sensor); push16(cpu); payload.push_back(profileBatt);
  ...
  MSPPacket pkt; pkt.cmd = MSP2_INAV_STATUS; pkt.payload = payload;
  decoder.decode(pkt);
  ...
  for (size_t i=0;i<activeModes.size();++i) EXPECT_EQ(received->active_modes[i], activeModes[i]);
...
TEST_F(InavStatusDecoderFixture, ShortPayloadIgnored) {
  ...
  MSPPacket pkt; pkt.cmd = MSP2_INAV_STATUS; pkt.payload = {0x00,0x01}; // too short
  decoder.decode(pkt);
  spin_for_messages(node_);
