...
TEST(AltitudeDecoder, BasicScaling) {
  ...
  auto sub_alt = node->create_subscription<std_msgs::msg::Float32>("/msp/altitude",10,[&](std_msgs::msg::Float32::SharedPtr m){alt_msg=m;});
  auto sub_vs = node->create_subscription<geometry_msgs::msg::TwistStamped>("/msp/vertical_speed",10,[&](geometry_msgs::msg::TwistStamped::SharedPtr m){vs_msg=m;});
  MSPPacket pkt; pkt.cmd = MSP_ALTITUDE;
...
TEST(AltitudeDecoder, ShortPayloadIgnored) {
  ...
  std_msgs::msg::Float32::SharedPtr alt_msg;
  auto sub_alt = node->create_subscription<std_msgs::msg::Float32>("/msp/altitude",10,[&](std_msgs::msg::Float32::SharedPtr m){alt_msg=m;});
  MSPPacket pkt; pkt.cmd = MSP_ALTITUDE; pkt.payload = {0x01,0x02,0x03};
  decoder.decode(pkt);
  spin_for(node);
