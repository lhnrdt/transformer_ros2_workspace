...
TEST(BatteryDecoder, AnalogBasic) {
  ...
  auto sub = node->create_subscription<sensor_msgs::msg::BatteryState>("/msp/battery",10,
    [&](sensor_msgs::msg::BatteryState::SharedPtr msg){ last = msg; });

  MSPPacket pkt; pkt.cmd = MSP_ANALOG; pkt.payload = { (uint8_t)123 }; // 12.3V
  decoder.decode(pkt);
  spin_for(node);
  ASSERT_TRUE(last);
...
TEST(BatteryDecoder, ExtendedState) {
  ...
  sensor_msgs::msg::BatteryState::SharedPtr last;
  auto sub = node->create_subscription<sensor_msgs::msg::BatteryState>("/msp/battery/dji",10,
    [&](sensor_msgs::msg::BatteryState::SharedPtr msg){ last = msg; });

  // Craft payload: cells=4, capacity(2)=5000 (-> 5 * 3600 = 18000C), mv(2)=1684 (16.84V), current(2)=1234 (12.34A), remaining%=85
  MSPPacket pkt; pkt.cmd = MSP_BATTERY_STATE; pkt.payload = {
    4,        // cells
