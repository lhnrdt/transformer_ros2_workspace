...
std::vector<uint8_t> buildPacketV2(uint16_t command, const std::vector<uint8_t> &payload, uint8_t flags) {
    ...
    uint8_t crc8 = 0;
    auto accumulate_crc = [&](uint8_t byte_value) { crc8 = update(crc8, byte_value); };
    ...
    frame_buffer.push_back(command_low_byte);  accumulate_crc(command_low_byte);
    frame_buffer.push_back(command_high_byte); accumulate_crc(command_high_byte);
    ...
    frame_buffer.push_back(length_low_byte);  accumulate_crc(length_low_byte);
    frame_buffer.push_back(length_high_byte); accumulate_crc(length_high_byte);
