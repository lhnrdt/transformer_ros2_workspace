...
void MSPParser::handleTunneledV2() {
  ...
  uint8_t crc = 0; for(size_t i=0;i<5;++i) crc = update(crc, buf_[i]);
  for(size_t i=0;i<inner_len;++i) crc = update(crc, buf_[5+i]);
...
void MSPParser::input(uint8_t b) {
  switch(state_) {
    ...
    case State::V2_Flags:
  frame_bytes_.push_back(b); v2_flags_ = b; v2_crc_calc_ = update(v2_crc_calc_, b); state_ = State::V2_IdLow; break;
    case State::V2_IdLow:
  frame_bytes_.push_back(b); v2_id_ = b; v2_crc_calc_ = update(v2_crc_calc_, b); state_ = State::V2_IdHigh; break;
    case State::V2_IdHigh:
  frame_bytes_.push_back(b); v2_id_ |= (uint16_t)b << 8; v2_crc_calc_ = update(v2_crc_calc_, b); state_ = State::V2_LenLow; break;
    case State::V2_LenLow:
  frame_bytes_.push_back(b); v2_len_ = b; v2_crc_calc_ = update(v2_crc_calc_, b); state_ = State::V2_LenHigh; break;
    case State::V2_LenHigh:
  frame_bytes_.push_back(b); v2_len_ |= (uint16_t)b << 8; v2_crc_calc_ = update(v2_crc_calc_, b); buf_.clear(); buf_.reserve(v2_len_); state_ = (v2_len_==0)?State::V2_CRC:State::V2_Payload; break;
    case State::V2_Payload:
  frame_bytes_.push_back(b); buf_.push_back(b); v2_crc_calc_ = update(v2_crc_calc_, b); if (buf_.size()>=v2_len_) state_ = State::V2_CRC; break;
    case State::V2_CRC:
