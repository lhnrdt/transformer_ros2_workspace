...
namespace transformer_msp_bridge
{
  ...
  {
    if (pkt.cmd == MSP2_SENSOR_RANGEFINDER)
      decodeRangefinder(pkt);
    else if (pkt.cmd == MSP2_SENSOR_COMPASS)
      decodeCompass(pkt);
    else if (pkt.cmd == MSP2_SENSOR_BAROMETER)
      decodeBarometer(pkt);
  }
  ...
  void SensorDecoder::decodeRangefinder(const MSPPacket &pkt)
  {
    ...
    if (debug_ && !logged_rangefinder_)
    {
      RCLCPP_INFO(rclcpp::get_logger("SensorDecoder"), "Rangefinder first frame: quality=%u dist=%.3fm", rf.quality, msg.data);
  ...
  void SensorDecoder::decodeCompass(const MSPPacket &pkt)
  {
    ...
    if (debug_ && !logged_compass_)
    {
      RCLCPP_INFO(rclcpp::get_logger("SensorDecoder"), "Compass first frame: inst=%u mGauss=(%d,%d,%d)", c.instance, c.magX, c.magY, c.magZ);
  ...
  {
    mspSensorBaroDataMessage_t b{};
    if (!decodeStruct(pkt, b))
      return;
    std_msgs::msg::Float32 m;
    m.data = b.pressurePa;
    barometer_pub_->publish(m);
    if (debug_ && !logged_baro_)
    {
      RCLCPP_INFO(rclcpp::get_logger("SensorDecoder"), "Barometer first frame: inst=%u pressure=%.2fPa temp=%.2fC", b.instance, b.pressurePa, b.temp / 100.0f);
      logged_baro_ = true;
    }
  }
